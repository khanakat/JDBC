...
Este capítulo proporciona un ejemplo sobre cómo eliminar una base de datos existente mediante la aplicación JDBC. Antes de ejecutar el siguiente ejemplo, asegúrese de tener lo siguiente en su lugar:

- Para ejecutar el siguiente ejemplo, debe reemplazar el nombre de usuario y la contraseña con su nombre de usuario y contraseña reales.

- Su MySQL o cualquier base de datos que esté utilizando está en funcionamiento.

**NOTA**: Esta es una operación seria y debes tomar una decisión firme antes de proceder a eliminar una base de datos porque todo lo que tienes en tu base de datos se perdería.
...
- **Abrir una conexión**: Requiere usar el método `DriverManager.getConnection()` para crear un objeto Connection, que representa una conexión física con un servidor de base de datos.  
  Eliminar una base de datos no requiere que el nombre de la base de datos esté en la URL de su base de datos. El siguiente ejemplo eliminaría la base de datos de ESTUDIANTES.

- **Ejecutar una consulta**: Requiere usar un objeto de tipo Statement para construir y enviar una sentencia SQL para borrar la base de datos.
